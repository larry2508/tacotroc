/*
 * package tacotroc.api.tacotroc.sécurity;
 * 
 ...
 * X509AuthenticationFilter; import
 ...
 * matchIfMissing = true)
 * 
 * @ComponentScan(basePackageClasses = KeycloakSecurityComponents.class) public
 * static class KeycloakConfigurationAdapter extends
 * KeycloakWebSecurityConfigurerAdapter {
 * 
 * @Bean
 * 
 * @Override protected SessionAuthenticationStrategy
 * sessionAuthenticationStrategy() { // TODO Auto-generated method stub return
 * null; }
 *//**
 ...
 * keycloakAuthenticationProvider = keycloakAuthenticationProvider(); // simple
 * Authority Mapper to avoid ROLE_
 ...
 *//**
	 ...
	 *//*
		 ...
		 * // disable csrf because of API mode .csrf().disable() .sessionManagement() //
		 * use previously declared bean
		 * .sessionAuthenticationStrategy(sessionAuthenticationStrategy())
		 * .sessionCreationPolicy(SessionCreationPolicy.STATELESS) // keycloak filters
		 * for securisation .and() .addFilterBefore(keycloakPreAuthActionsFilter(),
		 * LogoutFilter.class)
		 * .addFilterBefore(keycloakAuthenticationProcessingFilter(),
		 * X509AuthenticationFilter.class)
		 * .exceptionHandling().authenticationEntryPoint(authenticationEntryPoint()) //
		 * delegate logout endpoint to spring security .and() .logout()
		 * .addLogoutHandler(keycloakLogoutHandler())
		 * .logoutUrl("/logout").logoutSuccessHandler( // logout handler for API
		 * (HttpServletRequest request, HttpServletResponse response, Authentication
		 * authentication) -> response.setStatus(HttpServletResponse.SC_OK) ) .and() //
		 * manage routes securisation here .authorizeRequests()
